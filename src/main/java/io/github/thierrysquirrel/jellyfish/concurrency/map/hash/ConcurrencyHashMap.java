...
package io.github.thierrysquirrel.jellyfish.concurrency.map.hash;
...
public class ConcurrencyHashMap<K, V> extends AbstractMap<K, V> {
    ...
    public Map<K, V> getAll() {
        return super.getAllMap(new HashMap<>());
    }
    ...
    protected void init(int offset) {
        ...
        Map<K, V>[] thisMap = new HashMap[offset];
        ReadWriteLock[] thisLock = new ReadWriteLock[offset];

        for (int i = 0; i < offset; i++) {
            thisMap[i] = new HashMap<>();
